...
from contract_preprocess.core.declarations import (
    Contract,
    Enum,
    Event,
    Function,
...
from contract_preprocess.core.declarations.solidity_import_placeholder import SolidityImportPlaceHolder
from contract_preprocess.core.declarations.solidity_variables import SolidityCustomRevert
from contract_preprocess.core.expressions import Identifier, Literal
from contract_preprocess.core.expressions.expression import Expression
from contract_preprocess.core.solidity_types import (
    ArrayType,
    ElementaryType,
    FunctionType,
    MappingType,
...
from contract_preprocess.core.solidity_types.type_alias import TypeAliasTopLevel, TypeAlias
...
from contract_preprocess.ir.operations.codesize import CodeSize
from contract_preprocess.ir.tmp_operations.argument import Argument, ArgumentType
from contract_preprocess.ir.tmp_operations.tmp_call import TmpCall
from contract_preprocess.ir.tmp_operations.tmp_new_array import TmpNewArray
from contract_preprocess.ir.tmp_operations.tmp_new_contract import TmpNewContract
from contract_preprocess.ir.tmp_operations.tmp_new_elementary_type import TmpNewElementaryType
from contract_preprocess.ir.tmp_operations.tmp_new_structure import TmpNewStructure
from contract_preprocess.ir.variables import Constant, ReferenceVariable, TemporaryVariable
from contract_preprocess.ir.variables import TupleVariable
from contract_preprocess.utils.function import get_function_id, get_event_id
from contract_preprocess.utils.type import export_nested_types_from_variable
from contract_preprocess.utils.using_for import USING_FOR
from contract_preprocess.visitors.ir.expression_to_ir import ExpressionToIR
...
def convert_expression(expression: Expression, node: "Node") -> List[Operation]:
    ...
    visitor = ExpressionToIR(expression, node)
...
def _convert_type_contract(ir: Member) -> Assignment:
    ...
    raise IRError(f"type({contract.name}).{ir.variable_right} is unknown")
    ...
    if isinstance(ir, OperationWithLValue) and ir.lvalue:
        ...
        if not ir.lvalue.type:
            if isinstance(ir, Assignment):
                ...
                #
                # return_addr().balance
                # Here the IR builder will incorrectly create a REF variable instead of a Temp variable
                ...
                raise IRError(f"Not handling {type(ir)} during type propagation")
    return None
...
def extract_tmp_call(ins: TmpCall, contract: Optional[Contract]) -> Union[Call, Nop]:
    ...
    if isinstance(ins.ori, Member):
        ...
        if isinstance(ins.ori.variable_left, Function):
            ...
            #     }
            # }
            to_log = "ContractPreprocess does not support dynamic functions to libraries if functions have the same name"
            to_log += f"{[candidate.full_name for candidate in candidates]}"
            raise IRError(to_log)
    ...
    raise IRError(f"Not extracted {type(ins.called)} {ins}")
...
def convert_to_low_level(
    ...
    if ir.function_name in ["call", "delegatecall", "callcode", "staticcall", "raw_call"]:
        ...
        return new_ir
    raise IRError(f"Incorrect conversion to low level {ir}")
...
def convert_to_solidity_func(
    ...
    # abi.decode(a, (uint))
        ...
        # See https://github.com/crytic/contract_preprocess/issues/566 for potential solutions
...
def apply_ir_heuristics(irs: List[Operation], node: "Node", is_solidity: bool) -> List[Operation]:
    """
    Apply a set of heuristic to improve the IR
    """
