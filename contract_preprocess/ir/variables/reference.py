...
from contract_preprocess.core.declarations import Contract, Enum, SolidityVariable, Function
...
class ReferenceVariable(Variable):
    def __init__(self, node: "Node", index: Optional[int] = None) -> None:
        super().__init__()
        if index is None:
            self._index = node.compilation_unit.counter_ir_reference
            node.compilation_unit.counter_ir_reference += 1
        else:
            self._index = index
