...
import contract_preprocess.ir.variables.tuple_ssa
...
from contract_preprocess.core.declarations.modifier import Modifier
...
###################################################################################
###################################################################################
# region IR variables to SSA
###################################################################################
###################################################################################


def transform_ir_vars_to_ssa(
    function: Union[FunctionContract, Modifier, FunctionTopLevel]
) -> None:
    """
    Transform IR vars to SSA (TemporaryVariable, ReferenceVariable, TupleVariable)
    """
    variables = []
...
def add_ssa_ir(
    ...
    all_state_variables_instances: Dict[
        str, contract_preprocess.ir.variables.state_variable.StateIRVariable
    ],
) -> None:
    """
...
def copy_ir(ir: Operation, *instances) -> Operation:
    ...
    raise IRError(f"Impossible ir copy on {ir} ({type(ir)})")
