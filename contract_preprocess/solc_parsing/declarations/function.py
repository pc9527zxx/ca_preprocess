...
from contract_preprocess.core.cfg.node import NodeType, link_nodes, insert_node, Node
from contract_preprocess.core.cfg.scope import Scope
from contract_preprocess.core.declarations.contract import Contract
from contract_preprocess.core.declarations.function import (
    Function,
    ModifierStatements,
    FunctionType,
)
from contract_preprocess.core.declarations.function_contract import FunctionContract
from contract_preprocess.core.expressions import AssignmentOperation
from contract_preprocess.core.source_mapping.source_mapping import Source
from contract_preprocess.core.variables.local_variable import LocalVariable
from contract_preprocess.core.variables.local_variable_init_from_tuple import LocalVariableInitFromTuple
from contract_preprocess.solc_parsing.cfg.node import NodeSolc
...
from contract_preprocess.solc_parsing.variables.local_variable import LocalVariableSolc
from contract_preprocess.solc_parsing.variables.local_variable_init_from_tuple import (
    LocalVariableInitFromTupleSolc,
)
from contract_preprocess.solc_parsing.variables.variable_declaration import MultipleVariablesDeclaration
from contract_preprocess.utils.expression_manipulations import SplitTernaryExpression
from contract_preprocess.visitors.expression.export_values import ExportValues
from contract_preprocess.visitors.expression.has_conditional import HasConditional
from contract_preprocess.solc_parsing.yul.parse_yul import YulBlock
...
class FunctionSolc(CallerContextExpression):
    ...
    def __init__(
        ...
        self._parser: "SolcCompilationUnitParser" = parser
    ...
    def _analyze_attributes(self) -> None:
        ...
        if "baseFunctions" in attributes:
            ...
            if len(overrides_ids) > 0:
                for f_id in overrides_ids:
                    funcs = self.parser.functions_by_id[f_id]
                    for f in funcs:
        ...
        if "overrides" in attributes and isinstance(attributes["overrides"], dict):
            for override in attributes["overrides"].get("overrides", []):
                ...
                overridden_contract = self.parser.contracts_by_id.get(refId, None)
    ...
    def _parse_try_catch(self, statement: Dict, node: NodeSolc, scope: Scope) -> NodeSolc:
        ...
        if externalCall is None:
            raise ParsingError(f"Try/Catch not correctly parsed by ContractPreprocess {statement}")
    ...
    def _parse_catch(
        ...
        if block is None:
            raise ParsingError(f"Catch not correctly parsed by ContractPreprocess {statement}")
