...
from contract_preprocess.core.cfg.node import NodeType, Node, link_nodes
...
from contract_preprocess.core.declarations.function import FunctionLanguage
...
from contract_preprocess.solc_parsing.yul.evm_functions import (
...
from contract_preprocess.solc_parsing.expressions.find_variable import find_top_level
from contract_preprocess.visitors.expression.find_calls import FindCalls
from contract_preprocess.visitors.expression.read_var import ReadVar
from contract_preprocess.visitors.expression.write_var import WriteVar
...
class YulFunction(YulScope):
    ...
    def new_node(self, node_type: NodeType, src: str) -> YulNode:
        if self._function:
            ...
            raise PreprocessException("standalone yul objects are not supported yet")
...
def parse_yul_function_call(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if not isinstance(ident, Identifier):
        raise PreprocessException("expected identifier from parsing function name")
    ...
    raise PreprocessException(f"unexpected function call target type {str(type(ident.value))}")
...
def _parse_yul_magic_suffixes(name: str, root: YulScope) -> Optional[Expression]:
    ...
    # Currently IR doesnt support raw access to memory
    ...
    if name.endswith(".length"):
        ...
        # The code below is an hotfix to allow contract_preprocess to process length in yul
...
def parse_yul_identifier(root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    raise PreprocessException(f"unresolved reference to identifier {name}")
...
def parse_yul_unsupported(_root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    raise PreprocessException(f"no parser available for {ast['nodeType']} {json.dumps(ast, indent=2)}")
