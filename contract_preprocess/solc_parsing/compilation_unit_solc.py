...
from contract_preprocess.core.declarations import Contract, Function
from contract_preprocess.core.declarations.custom_error_top_level import CustomErrorTopLevel
from contract_preprocess.core.declarations.enum_top_level import EnumTopLevel
from contract_preprocess.core.declarations.event_top_level import EventTopLevel
from contract_preprocess.core.declarations.function_top_level import FunctionTopLevel
from contract_preprocess.core.declarations.import_directive import Import
from contract_preprocess.core.declarations.pragma_directive import Pragma
from contract_preprocess.core.declarations.structure_top_level import StructureTopLevel
from contract_preprocess.core.declarations.using_for_top_level import UsingForTopLevel
from contract_preprocess.core.scope.scope import FileScope
from contract_preprocess.core.solidity_types import ElementaryType, TypeAliasTopLevel
...
from contract_preprocess.solc_parsing.declarations.contract import ContractSolc
from contract_preprocess.solc_parsing.declarations.custom_error import CustomErrorSolc
from contract_preprocess.solc_parsing.declarations.function import FunctionSolc
from contract_preprocess.solc_parsing.declarations.event_top_level import EventTopLevelSolc
from contract_preprocess.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
from contract_preprocess.solc_parsing.declarations.using_for_top_level import UsingForTopLevelSolc
from contract_preprocess.solc_parsing.exceptions import VariableNotFound
from contract_preprocess.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

logging.basicConfig()
logger = logging.getLogger("SolcParsing")
logger.setLevel(logging.INFO)


class InheritanceResolutionError(PreprocessException):
    pass
...
def _handle_import_aliases(
    ...
    for symbol_alias in symbol_aliases:
        if "foreign" in symbol_alias and "local" in symbol_alias:
            ...
            # see https://github.com/crytic/contract_preprocess/issues/1319
...
class SolcCompilationUnitParser(CallerContextExpression):
    # pylint: disable=too-many-instance-attributes
    def __init__(self, compilation_unit: CompilationUnitWrapper) -> None:
        super().__init__()

        self._compilation_unit: CompilationUnitWrapper = compilation_unit
    ...
    def parse_top_level_items(self, data_loaded: Dict, filename: str) -> None:
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                raise PreprocessException(f"Top level {top_level_data[self.get_key()]} not supported")
        ...
        def resolve_remapping_and_renaming(contract_parser: ContractSolc, want: str) -> Contract:
            ...
            if target == contract_parser.underlying_contract:
                raise InheritanceResolutionError(
                    "Could not resolve contract inheritance. This is likely caused by an import renaming that collides with existing names (see https://github.com/crytic/contract_preprocess/issues/1758)."
                    f"\n Try changing `contract {target}` ({target.source_mapping}) to a unique name as a workaround."
                    "\n Please share the source code that caused this error here: https://github.com/crytic/contract_preprocess/issues/"
                )
    ...
    def _analyze_top_level_structures(self) -> None:
        try:
            ...
            raise PreprocessException(f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_top_level_variables(self) -> None:
        try:
            ...
            raise PreprocessException(f"Missing {e} during variable analyze") from e
    ...
    def _analyze_top_level_events(self) -> None:
        try:
            ...
            raise PreprocessException(f"Missing event {e} during top level event analyze") from e
    ...
    def _analyze_content_top_level_function(self) -> None:
        try:
            ...
            raise PreprocessException(f"Missing {e} during top level function analyze") from e
    ...
    def _convert_to_ir(self) -> None:
        ...
        for contract in self._compilation_unit.contracts:
            ...
            for func in contract.functions + contract.modifiers:
                try:
                    ...
                    logger.error(
                        f"\nFailed to generate IR for {contract.name}.{func.name}. Please open an issue https://github.com/crytic/contract_preprocess/issues.\n{contract.name}.{func.name} ({func.source_mapping}):\n "
                        f"{func_expressions}"
                    )
                    raise e
            try:
                ...
                logger.error(
                    f"\nFailed to convert IR to SSA for {contract.name} contract. Please open an issue https://github.com/crytic/contract_preprocess/issues.\n "
                )
                raise e
        ...
        for func in self._compilation_unit.functions_top_level:
            try:
                ...
                func_expressions = "\n".join([f"\t{ex}" for ex in func.expressions])
                logger.error(
                    f"\nFailed to generate IR for top level function {func.name}. Please open an issue https://github.com/crytic/contract_preprocess/issues.\n{func.name} ({func.source_mapping}):\n "
                    f"{func_expressions}"
                )
                raise e

            try:
                func.generate_ir_ssa({})
            except Exception as e:
                func_expressions = "\n".join([f"\t{ex}" for ex in func.expressions])
                logger.error(
                    f"\nFailed to convert IR to SSA for top level function {func.name}. Please open an issue https://github.com/crytic/contract_preprocess/issues.\n{func.name} ({func.source_mapping}):\n "
                    f"{func_expressions}"
                )
                raise e
