...
from contract_preprocess.core import expressions
...
from contract_preprocess.core.expressions.binary_operation import BinaryOperation
from contract_preprocess.core.expressions.expression import Expression
from contract_preprocess.core.expressions.index_access import IndexAccess
from contract_preprocess.core.expressions.literal import Literal
from contract_preprocess.core.expressions.new_array import NewArray
from contract_preprocess.core.expressions.new_contract import NewContract
from contract_preprocess.core.expressions.tuple_expression import TupleExpression
from contract_preprocess.core.expressions.unary_operation import UnaryOperation
from contract_preprocess.core.solidity_types import ArrayType, ElementaryType, TypeAlias, UserDefinedType
...
from contract_preprocess.ir.tmp_operations.argument import Argument
...
from contract_preprocess.ir.variables import (
    Constant,
    ReferenceVariable,
    TemporaryVariable,
    TupleVariable,
...
logger = logging.getLogger("VISTIOR:ExpressionToIR")
...
def convert_assignment(
    ...
    raise IRError("Missing type during assignment conversion")


class ExpressionToIR(ExpressionVisitor):

    # pylint: disable=super-init-not-called
    def __init__(self, expression: Expression, node: "Node") -> None:
        from contract_preprocess.core.cfg.node import NodeType  # pylint: disable=import-outside-toplevel
    ...
    def _post_assignement_operation(self, expression: AssignmentOperation) -> None:
        ...
        # (uint a,,) = g()
            ...
            # Special case for init of array, when the right has only one element e.g. arr = [1];
                ...
                # (see `contract_preprocess.vyper_parsing.type_parsing.parse_type`)
                args = []
    ...
    def _post_conditional_expression(self, expression: ConditionalExpression) -> None:
        raise IRError(f"Ternary operator are not convertible to IR {expression}")
    ...
    def _post_unary_operation(self, expression: UnaryOperation) -> None:
        ...
        if expression.type in [UnaryOperationType.BANG, UnaryOperationType.TILD]:
            ...
            raise IRError(f"Unary operation to IR not supported {expression}")
