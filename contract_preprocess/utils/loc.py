...
from contract_preprocess.utils.myprettytable import MyPrettyTable
from contract_preprocess.utils.tests_pattern import is_test_file
...
def compute_loc_metrics(contract_preprocess: ContractPreprocess) -> LoC:
    """Used to compute the lines of code metrics for a ContractPreprocess object.

    Args:
        contract_preprocess: A ContractPreprocess object
    Returns:
        A LoC object
    """
    ...
    for filename, source_code in contract_preprocess.source_code.items():
        current_lines = source_code.splitlines()
        is_dep = False
        if contract_preprocess.crytic_compile:
            is_dep = contract_preprocess.crytic_compile.is_dependency(filename)
