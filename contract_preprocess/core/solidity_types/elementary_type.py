...
class ElementaryType(Type):
    ...
    def size(self) -> Optional[int]:
        ...
        raise PreprocessException(f"{t} does not have a size")
    ...
    def min(self) -> int:
        ...
        raise PreprocessException(f"{self.name} does not have a min value")

    @property
    def max(self) -> int:
        if self.name in MaxValues:
            return MaxValues[self.name]
        raise PreprocessException(f"{self.name} does not have a max value")
