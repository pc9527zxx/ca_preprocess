...
from contract_preprocess.core.declarations.contract_level import ContractLevel
from contract_preprocess.core.compilation_unit import CompilationUnitWrapper
from contract_preprocess.core.context.context import Context
...
from contract_preprocess.core.source_mapping.source_mapping import SourceMapping, Source
from contract_preprocess.ir.variables import Constant
from contract_preprocess.utils.colors import red
from contract_preprocess.utils.sarif import read_triage_info
from contract_preprocess.utils.source_mapping import get_definition, get_references, get_all_implementations

logger = logging.getLogger("ContractPreprocess")
logging.basicConfig()
...
class Core(Context):
    """
    ContractPreprocess static analyzer
    """
    ...
    def __init__(self) -> None:
        ...
        self._previous_results_filename: str = "contract_preprocess.db.json"
        ...
        # Every contract_preprocess object has a list of result from detector
        ...
        # If set to true, contract_preprocess will not catch errors during parsing
        ...
        self._compilation_units: List[CompilationUnitWrapper] = []
    ...
    @property
    def compilation_units(self) -> List[CompilationUnitWrapper]:
        return list(self._compilation_units)

    def add_compilation_unit(self, compilation_unit: CompilationUnitWrapper):
    ...
    def parse_ignore_comments(self, file: str) -> None:
        ...
        while True:
            ...
            start_regex = r"^\s*//\s*contract_preprocess-disable-start\s*([a-zA-Z0-9_,-]*)"
            end_regex = r"^\s*//\s*contract_preprocess-disable-end\s*([a-zA-Z0-9_,-]*)"
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            ...
                            logger.error(
                                f"Consecutive contract_preprocess-disable-starts without contract_preprocess-disable-end in {file}#{line_number}"
                            )
                            return
            ...
            if end_match:
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            logger.error(
                                f"contract_preprocess-disable-end without contract_preprocess-disable-start in {file}#{line_number}"
                            )
                            return
    ...
    def has_ignore_comment(self, r: Dict) -> bool:
        ...
        for file, lines in mapping_elements_with_lines:
            ...
            if ignore_line_text:
                match = re.findall(
                    r"^\s*//\s*contract_preprocess-disable-next-line\s*([a-zA-Z0-9_,-]*)",
        ...
        for path in paths:
            try:
                ...
                logger.error(
                    ...
                    "\nThis tool supports the Python re format"
