...
from contract_preprocess.all_exceptions import PreprocessException
from contract_preprocess.core.declarations import Contract, Function, FunctionContract
...
from contract_preprocess.core.solidity_types import ElementaryType
...
from contract_preprocess.ir.convert import convert_expression
...
from contract_preprocess.ir.utils.utils import RVALUE
...
class Node(SourceMapping):  # pylint: disable=too-many-public-methods
    ...
    def __init__(
        ...
        self._ssa_vars_written: List["IRVariable"] = []
        self._ssa_vars_read: List["IRVariable"] = []
        ...
        self._ir_vars: Set[
            Union["IRVariable", ReferenceVariable, TemporaryVariable, TupleVariable]
        ] = set()  # non SSA
    ...
    @property
    def ssa_variables_read(self) -> List["IRVariable"]:
        """
        list(Variable): Variables read (local/state/solidity)
        """
    ...
    def ir_variables(
        self,
    ) -> List[Union["IRVariable", ReferenceVariable, TemporaryVariable, TupleVariable]]:
        return list(self._ir_vars)
    ...
    @property
    def ssa_variables_written(self) -> List["IRVariable"]:
        """
        list(Variable): Variables written (local/state/solidity)
        """
    ...
    def irs_ssa(self) -> List[Operation]:
        """Returns the IR representation with SSA
        ...
        if self._all_ir_operations is None:
            ...
            for ir in self.irs:
                if isinstance(ir, InternalCall):
                    irs += ir.function.all_ir_operations()
            self._all_ir_operations = irs
        return self._all_ir_operations

    @staticmethod
    def _is_non_ir_var(var: Variable) -> bool:
        return not isinstance(var, (Constant, ReferenceVariable, TemporaryVariable, TupleVariable))

    @staticmethod
    def _is_valid_ir_var(var: Variable) -> bool:
    ...
    def _find_read_write_call(self) -> None:  # pylint: disable=too-many-statements
        ...
        for ir in self.irs:
            ...
            self._ir_vars |= {v for v in ir.read if self._is_valid_ir_var(v)}

            if isinstance(ir, OperationWithLValue):
                var = ir.lvalue
                if var and self._is_valid_ir_var(var):
                    # The type is checked by is_valid_ir_var
                    self._ir_vars.add(var)  # type: ignore

            if not isinstance(ir, (Phi, Index, Member)):
                self._vars_read += [v for v in ir.read if self._is_non_ir_var(v)]
                for var in ir.read:
                ...
                if var and self._is_non_ir_var(var):
                    self._vars_read.append(var)
                if isinstance(var, ReferenceVariable):
                    origin = var.points_to_origin
                    if self._is_non_ir_var(origin):
