...
def _compute_line(
    compilation_unit: "CompilationUnit", filename: Filename, start: int, length: int
    ...
    try:
        ...
        # See the GitHub issue https://github.com/crytic/contract_preprocess/issues/2296
        ...
        """
        # We still re-raise the exception as a PreprocessException here
        raise PreprocessException(msg) from None

    return list(range(start_line, end_line + 1)), starting_column, ending_column


def _convert_source_mapping(
    offset: str, compilation_unit: "CompilationUnit"
...
class SourceMapping(Context):
    ...
    def set_offset(
        self, offset: Union["Source", str], compilation_unit: "CompilationUnit"
    ) -> None:
    ...
    def add_reference_from_raw_source(
        self, offset: str, compilation_unit: "CompilationUnit"
    ) -> None:
