...
from contract_preprocess.core.cfg.scope import Scope
from contract_preprocess.core.declarations.solidity_variables import (
    SolidityFunction,
    SolidityVariable,
    SolidityVariableComposed,
)
from contract_preprocess.core.expressions import (
    Identifier,
    IndexAccess,
    MemberAccess,
    UnaryOperation,
...
from contract_preprocess.utils.type import convert_type_for_solidity_signature_to_string
from contract_preprocess.utils.utils import unroll
...
if TYPE_CHECKING:
    ...
    from contract_preprocess.core.cfg.node import Node, NodeType
    ...
    from contract_preprocess.ir.variables import LocalIRVariable
    from contract_preprocess.core.expressions.expression import Expression
    from contract_preprocess.ir.operations import (
        HighLevelCall,
        InternalCall,
        LibraryCall,
        LowLevelCall,
...
class Function(SourceMapping, metaclass=ABCMeta):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, compilation_unit: "CompilationUnit") -> None:
        ...
        # ir Temporary and references variables (but not SSA)
        self._ir_variables: Set["IRVariable"] = set()
        ...
        self._return_values: Optional[List["IRVariable"]] = None
        self._return_values_ssa: Optional[List["IRVariable"]] = None
        ...
        self._ir_operations: Optional[List["Operation"]] = None
        self._ir_ssa_operations: Optional[List["Operation"]] = None

        self._all_expressions: Optional[List["Expression"]] = None
        self._all_ir_operations: Optional[List["Operation"]] = None
        ...
        self._all_ir_variables: Optional[List["IRVariable"]] = None
    ...
    def name(self) -> str:
        ...
        if self._function_type == FunctionType.CONSTRUCTOR_VARIABLES:
            return "internalConstructorVariables"
        if self._function_type == FunctionType.CONSTRUCTOR_CONSTANT_VARIABLES:
            return "internalConstructorConstantVariables"
        return self._name
    ...
    def can_reenter(self, callstack: Optional[List[Union["Function", "Variable"]]] = None) -> bool:
        ...
        :return bool:
        """
        from contract_preprocess.ir.operations import Call

        if self._can_reenter is None:
            self._can_reenter = False
            for ir in self.all_ir_operations():
    ...
    def is_constructor_variables(self) -> bool:
        ...
        ContractPreprocess has inbuilt functions to hold the state variables initialization
        """
    ...
    def nodes_ordered_dominators(self) -> List["Node"]:
        ...
        # from contract_preprocess.core.cfg.node import NodeType
    ...
    @property
    def ir_variables(self) -> List["IRVariable"]:
        """
        Temporary and Reference Variables (not SSA form)
        """

        return list(self._ir_variables)
    ...
    @property
    def return_values(self) -> List["IRVariable"]:
        """
        list(Return Values): List of the return values
        """
        from contract_preprocess.core.cfg.node import NodeType
        from contract_preprocess.ir.operations import Return
        from contract_preprocess.ir.variables import Constant
    ...
    @property
    def return_values_ssa(self) -> List["IRVariable"]:
        """
    ...
    @property
    def ir_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the ir operations
        """
        if self._ir_operations is None:
            operationss = [n.irs for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._ir_operations = operations
        return self._ir_operations

    @property
    def ir_ssa_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the ir operations (SSA)
        """
        if self._ir_ssa_operations is None:
            operationss = [n.irs_ssa for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._ir_ssa_operations = operations
        return self._ir_ssa_operations
    ...
    def all_ir_variables(self) -> List["IRVariable"]:
        """recursive version of ir_variables"""
        if self._all_ir_variables is None:
            self._all_ir_variables = self._explore_functions(lambda x: x.ir_variables)
        return self._all_ir_variables
    ...
    def all_ir_operations(self) -> List["Operation"]:
        if self._all_ir_operations is None:
            self._all_ir_operations = self._explore_functions(lambda x: x.ir_operations)
        return self._all_ir_operations
    ...
    def _solidity_variable_in_binary(node: "Node") -> List[SolidityVariable]:
        from contract_preprocess.ir.operations.binary import Binary
    ...
    def _solidity_variable_in_internal_calls(node: "Node") -> List[SolidityVariable]:
        from contract_preprocess.ir.operations.internal_call import InternalCall
    ...
    def apply_visitor(self, Visitor: Callable) -> List:
        ...
        Args:
            Visitor: contract_preprocess.visitors
        Returns
    ...
    def ir_cfg_to_dot(self, filename: str):
        """
        Export the CFG to a DOT file. The nodes includes the Solidity expressions and the IRs
        :param filename:
        :return:
        """
        content = self.ir_cfg_to_dot_str()
        with open(filename, "w", encoding="utf8") as f:
            f.write(content)

    def ir_cfg_to_dot_str(self, skip_expressions: bool = False) -> str:
        """
    ...
    def _analyze_read_write(self) -> None:
        ...
        ir_variables = [x.ir_variables for x in self.nodes]
        ir_variables = [x for x in ir_variables if x]
        self._ir_variables = [item for sublist in ir_variables for item in sublist]
    ...
    ) -> Dict[str, Set["IRVariable"]]:
        # pylint: disable=too-many-locals,too-many-branches
        from contract_preprocess.ir.variables import ReferenceVariable
        from contract_preprocess.ir.operations import OperationWithLValue
        from contract_preprocess.core.cfg.node import NodeType
    ...
    def _unchange_phi(ir: "Operation") -> bool:
        from contract_preprocess.ir.operations import Phi, PhiCallback
    ...
    def _fix_phi_entry(
        ...
        from contract_preprocess.ir.variables import Constant, StateIRVariable, LocalIRVariable
    ...
    def fix_phi(
        ...
        from contract_preprocess.ir.operations import InternalCall, PhiCallback, Phi
        from contract_preprocess.ir.variables import StateIRVariable, LocalIRVariable
    ...
    def generate_ir_and_analyze(self) -> None:
    ...
    @abstractmethod
    def generate_ir_ssa(self, all_ssa_state_variables_instances):
        pass
