...
from contract_preprocess.utils.using_for import USING_FOR, merge_using_for
from contract_preprocess.core.declarations.function import Function, FunctionType, FunctionLanguage
from contract_preprocess.utils.erc import (
    ERC20_signatures,
    ERC165_signatures,
    ERC223_signatures,
    ERC721_signatures,
...
from contract_preprocess.utils.tests_pattern import is_test_contract
...
if TYPE_CHECKING:
    ...
    from contract_preprocess.ir.operations import HighLevelCall, LibraryCall
    from contract_preprocess.ir.variables.variable import IRVariable
    from contract_preprocess.core.variables import Variable, StateVariable
    ...
    from contract_preprocess.core.solidity_types import TypeAliasContract
...
class Contract(SourceMapping):  # pylint: disable=too-many-public-methods
    ...
    def ir_variables(self) -> List["IRVariable"]:
        """
        List all of the ir variables (non SSA)
        """
        ir_variabless = [f.ir_variables for f in self.functions + self.modifiers]  # type: ignore
        ir_variables = [item for sublist in ir_variabless for item in sublist]
        return list(set(ir_variables))
    ...
    def state_variables_used_in_reentrant_targets(
        ...
        if self._state_variables_used_in_reentrant_targets is None:
            ...
            for function in reentrant_functions:
                for ir in function.all_ir_operations():
    ...
    def is_upgradeable_proxy(self) -> bool:
        ...
        from contract_preprocess.ir.operations import LowLevelCall
    ...
    def is_incorrectly_constructed(self) -> bool:
        """
        Return true if there was an internal ContractPreprocess's issue when analyzing the contract
        :return:
        """
    ...
    def convert_expression_to_ir_ssa(self) -> None:
        """
        Assume generate_ir_and_analyze was called on all functions

        :return:
        """
        from contract_preprocess.ir.variables import StateIRVariable
        ...
        for func in self.functions + list(self.modifiers):
            func.generate_ir_ssa(all_ssa_state_variables_instances)
